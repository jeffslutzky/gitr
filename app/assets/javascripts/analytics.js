...
function trigger_collaborators_chart(){
  $.ajax({
    url: '/analytics/2',
    dataType: 'json'
  }).success(function(data){
    draw_user_commits_chart(data)
  });
}

function draw_project_collaborators_chart(my_data) {
...
function draw_user_commits_chart(my_data) {
  var margin = {top: 20, right: 20, bottom: 180, left: 40},
      width = 800 - margin.left - margin.right,
      height = 500 - margin.top - margin.bottom;
  ...
  var y = d3.scale.linear()
      .range([height, 0])
      .domain([0, d3.max(my_data.n_commits)]);
      // .range([height, 0]);
